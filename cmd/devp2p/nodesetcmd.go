...
func ethFilter(args []string) (nodeFilter, error) {
	...
	rinkeby := forkid.NewStaticFilter(params.RinkebyChainConfig, params.RinkebyGenesisHash)
	goerli := forkid.NewStaticFilter(params.GoerliChainConfig, params.GoerliGenesisHash)
	ropsten := forkid.NewStaticFilter(params.RopstenChainConfig, params.RopstenGenesisHash)
	...
	f := func(n nodeJSON) bool {
		...
		if args[0] == "mainnet" {
			if rinkeby(eth.ForkID) == nil {
				fmt.Println("rinkeby node")
				return false
			} else if goerli(eth.ForkID) == nil {
				fmt.Println("goerli node")
				return false
			} else if ropsten(eth.ForkID) == nil {
				fmt.Println("ropsten node")
				return false
			}
		}
